...
def get_coor_points(source_feats_npy, target_feats_npy, target_npy):
    dists = torch.cdist(to_tensor(source_feats_npy), to_tensor(target_feats_npy)) # (n, m)
    inds = torch.min(dists, dim=-1)[1].cpu().numpy()
    return target_npy[inds], inds


def vote(source_npy, target_npy, source_feats, target_feats, voxel_size):
    source_feats_h, source_feats_m, source_feats_l = source_feats
    target_feats_h, target_feats_m, target_feats_l = target_feats

    coor_y1, coor_inds1 = get_coor_points(source_feats_h, target_feats_h, target_npy)
    coor_y2, coor_inds2 = get_coor_points(source_feats_m, target_feats_m, target_npy)
    coor_y3, coor_inds3 = get_coor_points(source_feats_l, target_feats_l, target_npy)
